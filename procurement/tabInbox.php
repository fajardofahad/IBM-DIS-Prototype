...
$sql_rfq = 'Select * from rfq where procurementFolder = 0 ';
$sql_abn = 'Select * from abn where procurementFolder = 0 ';
$sql_dir = 'Select * from dir where procurementFolder = 0 ';
$sql_mir = 'Select * from mir where procurementFolder = 0 ';

//requestor specific
if(isset($_REQUEST['searchRequestor']) && $_REQUEST['searchRequestor'] != ""){
	$rt = 'and requestor like  \'%'.$_REQUEST['searchRequestor'].'%\' ';
	$sql_rfq .= $rt;
	$sql_abn .= $rt;
	$sql_dir .= $rt;
	$sql_mir .= $rt;
...
if($_REQUEST['searchRequestStatus'] != '-'){
	$rs = 'and procurementCheck = '.$_REQUEST['searchRequestStatus'].' ';
	$sql_rfq .= $rs;
	$sql_abn .= $rs;
	$sql_dir .= $rs;
	$sql_mir .= $rs;
...
*/
					if($row['procurementCheck'] == 0 || $row['procurementCheck'] == 5){
						$setColor = '#FFCC66';
						$imageMail = 'newMail.gif';
					}elseif($row['procurementCheck'] == 1){
						$setColor = '#999999';
						$imageMail = 'mailChecked.gif';
					}elseif($row['procurementCheck'] == 2){
						$setColor = '#666666';
		...
		echo '<a href="javascript:ajaxpage(\'ajaxfiles/external.php?idInquiry='.$requestNo.'&idRfqType='.$rfqType.'&idStatus='.$row['procurementCheck'].'\', \'rightcolumn\');" >
...
*/
		...
		echo '<a href="javascript:ajaxpage(\'ajaxfiles/external2.php?idInquiry='.$requestNo.'&idStatus='.$row['procurementCheck'].'\', \'rightcolumn\');" >
