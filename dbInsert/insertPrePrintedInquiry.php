...
$rfqType = 'Customized Pre-Printed Inquiry';
...
if($noOfItems == 1){
	...
	$insertChild = "Insert into rfq_pre_printed values('$Rn','$cmbForm','$quantity','$unitOfMeasure','$material','$size','$color','$printing','$DateNeeded','$noOfItems')";
	...
	for($i=1;$i<=$noOfItems;$i++){
		...
		$cmbForm  = $_POST['cmbForm'.$i];
		$quantity  = $_POST['quantity'.$i];
		$size  = $_POST['size'.$i];
		$color  = $_POST['color'.$i];
		$material  = $_POST['material'.$i];
		$unitOfMeasure  = $_POST['unitOfMeasure'.$i];
		$printing  = $_POST['printing'.$i];
		
		//UPLOADING OF FILE
		include('multiUpload.php');
		
		//INSERT ITEM TO DATABASE	
		$insertChild = "Insert into rfq_pre_printed values('$Rn','$cmbForm','$quantity','$unitOfMeasure','$material','$size','$color','$printing','$DateNeeded','$i')";
	$result = mysql_query($insertChild) or die (mysql_error());
...
//MESSAGE FOR HISTORY
$message = 'Custom : Pre-Printed Inquiry';
